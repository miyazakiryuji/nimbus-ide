...
**段は「直さないといけないか」で決める**（T-296）。宣言と実装のズレのように
**放っておくと壊れる**ものは `error`、重複やテストの無いモジュールのように
**直すかを人が決める**ものは `warn`（参考）。ここを取り違えると、毎回赤くなる検査ができて
**「赤いのが普通」になり、本物の指摘が埋もれる** — 実際に、動いているビューを
「登録していない」と言う偽の指摘（T-284）が、重複 57 件の中に紛れていた。
...
- **重複を機械で潰さない**（T-296）— 同名 export も、そっくりな実装も、意図してそうしていることがある。
  参考として並べるところまでにして、まとめるかは `refactor` スキルで人が決める。
  守りは「ドクターが要対応 0 件で終わること」そのもの（`test.sh` が落とすので、
  段を戻せばその場で分かる）
