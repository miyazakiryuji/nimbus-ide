...
- [ ] T-255 文脈とレビューをサイドバーの既定から外す — 常用の段を絞る。消さずに `visibility: hidden` にして、コマンド「Nimbus: 文脈を開く」「Nimbus: レビューを開く」から開けるようにする（実装済み・コミット待ち）[P2]
- [ ] T-256 タスクをアクティビティバーに独立させる — サイドバーの一段ではなく専用のアイコンにする。アイコンは雲＋一覧（雨＝Nimbus / 歯車＝設定 と質感で分ける）（実装済み・コミット待ち）[P1]
- [ ] T-257 タスクの状態表記を英語にする — `KANBAN_COLUMNS` の 未着手 / 作業中 / 完了 を To Do / In Progress / Done に。承認待ち・レビュー待ちも英語に揃える（混ざると読みにくい）[P2]
- [ ] T-263 承認待ちの置き場所を決める — T-266 で会話の中に出す方針になったので、一覧としての役割を「まだ答えていないものを横断で拾う」に絞れるか検討する [P2]
- [ ] T-266 **承認を Cursor のように会話の中で受ける** — いまはブロッキングのモーダル（`showWarningMessage({modal:true})`）＋別立ての「承認待ち」一覧。Cursor は**会話の流れの中にカードで出て、その場で許可・拒否**する。読んでいる場所と決める場所が同じなので、手も目も動かない（人間工学 E2）。コックピットは webview なので描画は可能。**待ち合わせの作り替え**が本体 — 許可の判定がモーダルの戻り値ではなく webview からの返事を待つ形になる。詰めるべきは (1) コックピットが閉じている／別ウィンドウのときの逃げ道（ステータスバーの件数＋一覧を残すか、モーダルへ落とすか）(2) 差分をカードの中に出すか「詳細を開く」に留めるか (3) 承認待ちのあいだ入力を止めない（いまはウィンドウごと止まる）[P1]
- [ ] T-264 セッションは作業前に必ず板へ書き出す（運用ルール）— 書き出していない作業は他セッションから見えず、同じ土台を二重に作る事故になる。`CLAUDE.md` に明文化済み。守られているかは、着手時に `@` の重なりを見て判断する [P1]
- [ ] T-265 ヘルプ（ゆあ）を「Nimbus 設定」へ移す — 迷ったときの入口は、設えを触る部屋にまとめる。常用サイドバーは会話と、走っているものに絞る。webview なので `hidden` にはできない（`focus` で出てこない）ため、設定側で `collapsed` に置く [P2]
- [ ] T-267 GUI テスト「置き換えの追跡が、実際の git grep から件数を出す」が**単独でも落ちる** — 当たらないパターンでも**先に名前を聞く**ので、ケースが名前を答えないまま入力欄で止まり、「当たる箇所がありません」が出ない（`refactorProgress.ts` はパターン → 名前 → 数える の順）。名前を聞く前に数えて、当たらなければ聞かずに断るのが素直（ケース側で名前を答える手もある）[P2]

## 進行中

- [ ] T-254 デバッガが止まった場所をセッションへ渡すボタン — 例外やテスト失敗で止まったその場のスタックと変数を、聞かれる前に渡せるようにする（T-104 は Claude 側から取りに行く口だけ）[P2] @session-d 2026-08-18

- [ ] T-258 コックピットとタスク板をエディタタブで開けるようにする — サイドバーの幅では狭い。`createWebviewPanel` でタブに出し、VS Code 内で全画面にできるようにする。サイドバー側と状態が二重にならない持ちかたを先に決める [P1] @session-d 2026-08-18

- [ ] T-259 タスクの追加・削除を、どのセッションからでも行えるようにする — いまは板の状態がウィンドウ内にしかない。**T-247 の永続化が前提**（土台を二重に作らないこと）[P1] @session-d 2026-08-18
- [ ] T-260 作業中のタスクに、担当しているセッションを記録する — セッションがタスクに着手したら状態を自動で「作業中」へ移し、どのセッションが持っているかを残す。**T-247 の持ち主決め（ロック）と同じ話**なので、そこに乗せる [P1] @session-d 2026-08-18
- [ ] T-261 タスクに実装の進捗を逐次記録する — 途中で止まったときに「どこまで何をしたか」が分かるように、タスクの中に追記していく。再開の手がかりにする。追記が壊れないことは T-250（追記専用の書き出し）と同じ問題 [P1] @session-d 2026-08-18
- [ ] T-262 タスクのヘルスチェックをボタン 1 つで — 途中で止まっているタスク、既に実装済みなのに残っているタスクを洗い出す。判定材料は最終更新時刻・担当セッションの生存（T-260）・進捗の記録（T-261）・作業ツリーの差分 [P2] @session-d 2026-08-18

- [ ] T-240 GUI テスト「コミットの分けかたが、実際の変更から束を出す」が**フル実行のときだけ落ちる** — 単独（`--only コミット`）では通る。前のケースが作業ツリーに残したものを拾っている可能性。ケース間で作業ツリーを戻すか、ケース専用の一時リポジトリを使う [P2] @session-d 2026-08-18
- [ ] T-247 **セッション管理のやり方を見直す** — 状態が `session/SessionManager.ts` のプロセス内 `Map` にしかなく、ウィンドウ（＝拡張ホスト）ごとに別の SessionManager が立つ。落ちたら復帰できず、複数ウィンドウから同じセッションを二重に触れる余地も残る。**持ち主を 1 つに決める（ロック）＋状態を永続化する**形へ寄せる。T-248 と対 [P1] @session-d 2026-08-18
- [ ] T-248 **Claude Code を並列に走らせたときの破損と遅延を防ぐ** — まず実測（同時 N セッションで何がどれだけ遅くなるか・どこが壊れるか）を取り、その上で共有ファイルへの書き込みの直列化・SDK 呼び出しの同時実行上限とバックオフ・重い処理の間引きを入れる。**対策より先に測る**（当てずっぽうで直さない）[P1] @session-d 2026-08-18
- [ ] T-250 監査ログの追記が「全部読んで全部書き直す」になっている（`extension.ts` の `appendAudit`）— 同時に書くと**行が消え**、ファイルが育つほど 1 行の追記が遅くなる。追記専用の書き出しに変える（T-248 の具体例・破損と遅延の両方に効く）[P1] @session-d 2026-08-18
- [ ] T-251 セッション横断の同時実行を 1 か所で絞る — F4 のタスクキューの外から起きたセッションが数に入っていない。「今いくつ走っているか」を 1 か所で持ち、上限と順番待ちを画面に出す [P2] @session-d 2026-08-18
- [ ] T-252 落ちた・閉じたあとのセッション復帰 — Claude 側の `session_id` は残るので、開き直したときに「続きから」を出せるはず。T-247 の永続化が前提 [P2] @session-d 2026-08-18
- [ ] T-253 セッションの作業ディレクトリが重なったら知らせる — 同じフォルダを 2 つのセッションが同時に編集すると、片方の前提が黙って壊れる。重なりを検出して警告し、worktree の切り出しを勧める [P2] @session-d 2026-08-18
