...
- [ ] T-240 GUI テスト「コミットの分けかたが、実際の変更から束を出す」が**フル実行のときだけ落ちる** — 単独（`--only コミット`）では通る。前のケースが作業ツリーに残したものを拾っている可能性。ケース間で作業ツリーを戻すか、ケース専用の一時リポジトリを使う [P2]
- [ ] T-247 **セッション管理のやり方を見直す** — 状態が `session/SessionManager.ts` のプロセス内 `Map` にしかなく、ウィンドウ（＝拡張ホスト）ごとに別の SessionManager が立つ。落ちたら復帰できず、複数ウィンドウから同じセッションを二重に触れる余地も残る。**持ち主を 1 つに決める（ロック）＋状態を永続化する**形へ寄せる。T-248 と対 [P1]
- [ ] T-248 **Claude Code を並列に走らせたときの破損と遅延を防ぐ** — まず実測（同時 N セッションで何がどれだけ遅くなるか・どこが壊れるか）を取り、その上で共有ファイルへの書き込みの直列化・SDK 呼び出しの同時実行上限とバックオフ・重い処理の間引きを入れる。**対策より先に測る**（当てずっぽうで直さない）[P1]
...
- [ ] T-250 監査ログの追記が「全部読んで全部書き直す」になっている（`extension.ts` の `appendAudit`）— 同時に書くと**行が消え**、ファイルが育つほど 1 行の追記が遅くなる。追記専用の書き出しに変える（T-248 の具体例・破損と遅延の両方に効く）[P1]
- [ ] T-251 セッション横断の同時実行を 1 か所で絞る — F4 のタスクキューの外から起きたセッションが数に入っていない。「今いくつ走っているか」を 1 か所で持ち、上限と順番待ちを画面に出す [P2]
- [ ] T-252 落ちた・閉じたあとのセッション復帰 — Claude 側の `session_id` は残るので、開き直したときに「続きから」を出せるはず。T-247 の永続化が前提 [P2]
- [ ] T-253 セッションの作業ディレクトリが重なったら知らせる — 同じフォルダを 2 つのセッションが同時に編集すると、片方の前提が黙って壊れる。重なりを検出して警告し、worktree の切り出しを勧める [P2]
