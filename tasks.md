...
- [ ] T-311 **セッションをピン留めできるようにする** — タブの上でピンを押すと先頭に残る。並びは「ピン留め（始めた順）→ それ以外（始めた順）→ 下書き」。**番号は並び替えても変えない**（番号は席順ではなく名札。変わると「2 番のセッション」という会話が壊れる）。ピンは**並びとしるしだけ** — 停止・後片付けの対象からは外さない（守る意味を持たせると「止めたはずが走っている」を生む）。状態は workspaceState に持ち、消えたセッションのぶんは掃除する [P2] @session-main 2026-08-25
- [ ] T-312 **ボタンに「何が起きるか」のヒントが無い** — カーソルを合わせても出ないという報告。実際に無いのは (1) **承認カードの 4 ボタン**（許可 / このセッションは許可 / 常に許可 / 拒否 — 押した結果の違いこそ説明が要るのに無い）(2) 折りたたみの見出し (3) タスク板の 5 ボタン（完了が worktree を片付けることは押す前に知りたい）。コンポーザー（送信・中断・添付）と view/title は既に有る。**ラベルの言い換えではなく、押した結果を書く** [P2] @session-main 2026-08-25
...
- [ ] T-315 **ボタン一つで CLAUDE.md を作る（テンプレートを何種類か用意する）** — 今は無いときに
      「節を足す」で 1 節ずつ作れるだけ（仕様 [claude-md](nimbus/docs/specs/claude-md.md)）で、
      **白紙から書き始めるのが重い**。サイドバー「CLAUDE.md」のタイトルに `$(new-file)`
      「CLAUDE.md を作る」を置き、押したらテンプレートを選んで作って開く。
      用意するテンプレート（案）: ① **最小**（何をするプロジェクトか / 触ってよい場所 / 走らせ方の
      3 見出しだけ）② **アプリ開発**（ビルド・テスト・依存の入れ方・環境変数）③ **並行開発**
      （コミットの作法・触らない場所・記録の直しかた — Nimbus 自身が使っている型）
      ④ **Flutter / モバイル**（実機とシミュレータ・ホットリロードの前提）⑤ **ライブラリ / 公開リポジトリ**
      （秘密を書かない・リリース手順）。**中身は数えた事実で埋める** — 「このリポジトリの書き方を数える」
      （`conventions.ts`）を差し込めば、雛形が最初から当たる（推測を書き込まない）。
      約束: **既にある CLAUDE.md を上書きしない**（追記か別名を提案）・どの階層に作るかを聞く
      （親から継承しているときは特に）・作ったら開いて人に読ませる。
      テンプレートは利用者が自分のぶんを足せる形に置く [P2]
