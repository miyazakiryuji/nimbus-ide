...
- 🔒 @session-h | T-339 | 2026-08-29 09:10 | extensions/nimbus/media/cockpit.css, extensions/nimbus/media/cockpit.js, nimbus/tests/gui/run.mjs, nimbus/docs/specs/cockpit-chat.md
...
- [ ] T-339 **コックピットで文字を範囲選択できるようにする**（利用者依頼 2026-08-29）— @session-h 2026-08-29
      実測（パッケージ版・本物のマウスドラッグ）で先に切り分けた: 段落の中／段落をまたぐ／ターンをまたぐ／
      逆向き／⌘A／左の余白 16px から／サイドバーと全画面タブの両面／⌘C→⌘V の往復 は **すでに全部通る**。
      流れている最中の選択も消えない。**選べないのは折りたたみの見出し**（`Bash · ls -la …` の行）— 
      `<button>` なのでドラッグ選択が構造的に不可能。写したい筆頭がここなので体感は「選べない」になる。
      併せて、文字の上のカーソルが `auto` で選べる手応えが無い。残る未測定の手つき（ダブルクリックで単語・
      右クリックでコピー・流れている最中に下端まで引く）も測ってから塞ぐ。
      済: 切り分け（実測 5 本）/ 残: 実装・回帰テスト / 次: 折りたたみ見出しを選べる形へ
