...
- [ ] T-316 **Herdr のように、セッション同士を束ねて関連づける** — Herdr は
      ...
      **セッション同士の関係を持っていない**（親子も、同じ仕事のグループも）。足すのは 2 つ:
      ...
      として扱う）。操作は Nimbus のセッションだけに効かせ、**Herdr のぶんは今までどおり読むだけ** [P1]

## 作業予約（いま触っているファイル）

編集を始める前にここへ 1 行足し、コミットと push が済んだら自分で行を消して解放する。
他人の行は編集も削除もしない。運用の詳細は CLAUDE.md「ファイルは板で予約してから触る」。

書式: `- 🔒 @session-x | T-123 | 2026-08-25 20:00 | 触るファイル（カンマ区切り）`

- 🔒 @session-h | T-322 | 2026-08-25 20:37 | nimbus/docs/README.md（新規）, nimbus/docs/specs/README.md
...
- [ ] T-319（旧 T-315・ID が重複したので採番し直し）**ボタン一つで CLAUDE.md を作る（テンプレートを何種類か用意する）** — 今は無いときに
      ...
      **白紙から書き始めるのが重い**。サイドバー「CLAUDE.md」のタイトルに `$(new-file)`
      ...
      テンプレートは利用者が自分のぶんを足せる形に置く [P2] @session-fable 2026-08-25
...
- [ ] T-322（旧 T-321・同時起票で ID が重複したので採番し直し）**現状を文書へ書き出す**（利用者依頼
      2026-08-25）— README の「できること」「使いかた」が F1〜F6 期の記述のまま（タブ列・会話内承認・
      使用量の帯・MCP 3 系統・IntelliJ 由来・画面の置きかたの変更が未反映）。仕様書の索引も
      2026-08-19 の棚卸し以降の 9 本（cockpit-home / cockpit-split / commit-message / git-sync /
      git-tools / run-settings / session-health / setup-readiness / typography）が漏れている。
      直すのは ① README の柱の書き直し ② 索引の追補 ③ `nimbus/docs/README.md` を新設
      （文書の歩きかた＋ 2026-08-25 時点の全体像）。**コードは触らない**（文書のみ・固め直し不要）。
      README.md は T-321 の予約解放を待ってから [P1] @session-h 2026-08-25

- [ ] T-323 **デッドコードの点検と除去**（利用者依頼 2026-08-25・T-322 の続き）— ドクターで
      「どこからも辿れないモジュール（orphans）・使われていない依存（deps）・参照されていない
      素材（assets）・宣言と実装のズレ（contributes）」を洗い出し、**確実に死んでいるものだけ**を
      消す。生きている重複の整理は提案止まり（参考 191 件は対象外）。進行中タスクの領域
      （cockpit まわり・SessionManager / sessionRegistry・board.mjs）と予約中のファイルには触らない。
      消す前に Before のテスト結果を控え、After で同じだけ通ることを確認。
      コードに触ったら固め直しまで [P1] @session-h 2026-08-25
