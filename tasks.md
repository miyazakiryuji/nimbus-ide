...
- [ ] T-303 **「+」がセッションを増やさない** — `newSession()` が `sessions.close()` で**いまのセッションを閉じてから**未開始状態にする実装なので、(1) タブが増えない (2) 押しても 2 番目が選ばれて見える (3) 押すだけ増やせない。さらに `cockpit.reveal()` を無条件に呼ぶので、**全画面タブで押すと閉じているサイドバーまで勝手に開く**。「+」はブラウザのタブと同じく**足す**操作にする。未開始のセッションもタブに出す（出ないと押した手応えが無い）[P1]
- [ ] T-304 セッションの中身がマークダウンのまま出ることがある — `parseMarkdown` は見出し・コード・箇条書き・引用・強調を扱うが、**表（table）が種類に無い**。Claude は表をよく出すので、そこが生のまま並んでいる疑い。実際に出力させて確かめてから足す [P2]
...
- [ ] T-308 **競合しているその場から Claude に相談できるようにする** — **中身は既にある**:
      ...
      **その競合の頭に CodeLens** を出す（`<<<<<<<` を見て出すので upstream に手を入れなくて済む）
      ...
      書き戻さない**、選ばれなかった競合はマーカーごと残す、保存と `git add` は人の手 [P2]
- [ ] T-309 **コミットメッセージの型を指定できるようにする**（T-305 が使う）— 生成ボタンが
      ...
      コミットの型は別に数える）。生成の前に「いまの型」を画面に出し、違えば押す前に直せること [P2]
