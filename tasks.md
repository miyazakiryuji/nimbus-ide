...
- [ ] T-310（旧 T-303・ID が重複したので採番し直し）**「+」がセッションを増やさない** — `newSession()` が `sessions.close()` で**いまのセッションを閉じてから**未開始状態にする実装なので、(1) タブが増えない (2) 押しても 2 番目が選ばれて見える (3) 押すだけ増やせない。さらに `cockpit.reveal()` を無条件に呼ぶので、**全画面タブで押すと閉じているサイドバーまで勝手に開く**。「+」はブラウザのタブと同じく**足す**操作にする。未開始のセッションもタブに出す（出ないと押した手応えが無い）[P1] @session-e 2026-08-25

- [ ] T-304 セッションの中身がマークダウンのまま出ることがある — `parseMarkdown` は見出し・コード・箇条書き・引用・強調を扱うが、**表（table）が種類に無い**。Claude は表をよく出すので、そこが生のまま並んでいる疑い。実際に出力させて確かめてから足す [P2] @session-e 2026-08-25
