...
- [ ] T-249 **デバッグ機能の改善と検討** — T-246 で標準デバッグをアクティビティバーから外したままなので、その後釜を決める。今あるのは MCP の `debug_stack` / `debug_variables`（T-104）だけ。例外で止まった場所を渡す・起動構成を作る・ブレークポイントを提案する、のどれを Claude 用の入口にするかを検討してから足す [P1]
...
- [ ] T-258 コックピットとタスク板をエディタタブで開けるようにする — サイドバーの幅では狭い。`createWebviewPanel` でタブに出し、VS Code 内で全画面にできるようにする。サイドバー側と状態が二重にならない持ちかたを先に決める [P1]
- [ ] T-259 タスクの追加・削除を、どのセッションからでも行えるようにする — いまは板の状態がウィンドウ内にしかない。**T-247 の永続化が前提**（土台を二重に作らないこと）[P1]
- [ ] T-260 作業中のタスクに、担当しているセッションを記録する — セッションがタスクに着手したら状態を自動で「作業中」へ移し、どのセッションが持っているかを残す。**T-247 の持ち主決め（ロック）と同じ話**なので、そこに乗せる [P1]
- [ ] T-261 タスクに実装の進捗を逐次記録する — 途中で止まったときに「どこまで何をしたか」が分かるように、タスクの中に追記していく。再開の手がかりにする。追記が壊れないことは T-250（追記専用の書き出し）と同じ問題 [P1]
- [ ] T-262 タスクのヘルスチェックをボタン 1 つで — 途中で止まっているタスク、既に実装済みなのに残っているタスクを洗い出す。判定材料は最終更新時刻・担当セッションの生存（T-260）・進捗の記録（T-261）・作業ツリーの差分 [P2]
