...
優先度や置き場所は後で決める。
- [ ] T-318 **開き直したら、開いていたセッションが全部消えた**（利用者報告 2026-08-25）— T-252 で「過去のセッションを再開する」（台帳から手動で続きから）は入ったが、**起動時に自動では何も戻らず**、タブ列も空なので全損に見える。直しは 2 段: (1) まず起動時に「前回のセッションが N 本あります — 再開 / 消す」を**見える場所**（コックピットの白紙面）に出す（コマンドを知らなくても辿り着ける・E4）(2) その先で、タブ列そのもの（ピン・名前・番号の台帳は既に workspaceState に残っている）を台帳と突き合わせて自動復元する。**SessionManager／sessionRegistry の領域なので T-247〜T-252 を作った側が適任** [P1]
- [ ] T-315 **セッションをソースコードと同じく画面分割で並べて見る** — いまはコックピットの面（サイドバー・エディタタブ）がすべて**同じアクティブセッションの鏡**で、2 面開いても同じものが映る。並べるには**面ごとに「どのセッションを見るか」を束ねる**作り替えが要る: (1) `WebviewViewHost` の post を面ごとに分ける (2) スナップショットと会話の記録をセッション別に引けるようにする (3) 入力・承認カードをその面のセッションへ向ける。エディタグループに置く器（`openInEditor`）は既に有るので、残りは配線の問題。大きいので設計から [P2]
...
- [ ] T-314 **コックピットに「タブ（束）」と Home を入れる** — 利用者とすり合わせた形（2026-08-25）:
      ...
      **UI 配線（cockpit.js / CockpitViewProvider / extension.ts）は T-311 / T-313 の
      未コミット作業と同じファイルなので、そちらが落ち着いてから**（板ルール）[P1] @session-e 2026-08-25
...
- [ ] T-305 **コミットメッセージを自動で作るボタン** — 置き場所は **SCM の入力欄**
      （`menus."scm/inputBox"`・VS Code 標準の「Generate Commit Message」と同じ場所。Copilot を
      外しているので**この口は空いている**）。コックピットからも呼べるコマンドにする。
      材料は `git diff --staged`。**勝手に `git add` しない** — 並行セッションでは他人の変更を
      巻き込むため、staged が空なら「先に組んでください」と言って終わる
      （束ね方の提案は既にある: [commit-split](nimbus/docs/specs/commit-split.md)）。
      **書きぶりはこのリポジトリの規約に従わせる** — 1 行目は日本語で `Nimbus: 〜（T-xxx）`。
      規約の読み取り（`conventions.ts`）と過去のコミットの型を材料にする。
      作ったものは**入力欄に入れるだけ**で、コミットも push もしない（人が読んでから）。
      本文は必ずサニタイザを通す（公開リポジトリなので資格情報・個人情報を載せない）。
      確かめるのは ① staged が空 ② 巨大な diff（切り詰めかた）③ 生成に使うモデルを選べるか
      （軽いモデルで足りる・T-291 と関連）[P2] @session-e 2026-08-25

- [ ] T-306 **コミットのあと、取り込みと押し上げをボタンでできるようにする**（fetch / rebase /
      pull / push）— **標準の口を先に使う**: VS Code の Git 拡張には `git.postCommitCommand`
      （`none` / `push` / `sync`・既定 `none`）と `git.rebaseWhenSync`（既定 `false`）、
      `git.autofetch`（既定 `false`）があり、SCM の同期ボタンも既にある。**作り直さない**。
      足りないのは 2 つだけ: ① **Nimbus の作法は `git pull --rebase` → `git push`** なので、
      既定を規約に合わせる（`postCommitCommand: sync` ＋ `rebaseWhenSync: true` ＋ autofetch）
      ② **並行セッションの安全** — 他セッションの未コミット変更が作業ツリーに居ると rebase は
      止まる。ここで **autostash を勝手に使わない**（他人の変更を触ることになる）。止まったら
      理由と次の一手（誰の変更が残っているか）を出す。衝突したときも黙って続けない。
      押す場所は SCM の同期ボタンと、コックピット側にも 1 つ（T-294 の入口の棚卸しに合流）。
      確かめるのは ① 何も無いときに押しても壊れない ② 衝突したときに止まって説明が出る
      ③ 追跡していないブランチで押したときの挙動 [P2] @session-e 2026-08-25

- [ ] T-307 **Git の操作を Nimbus のセッションからできるようにする**（`nimbus_git`・まず検討）—
      今ある MCP は `nimbus_debug`（停止位置のスタックと変数）と `nimbus_lsp` だけで、git は無い。
      **注意**: 素の `git` は Bash で打てるので、ただのラッパーは選ぶ基準 1（Claude Code 単体で
      できることは作らない）に引っかかる。足す価値があるのは **Nimbus 側にしか無いもの** の 4 つ:
      ① **並行セッションの作法を強制する** — `git add -A` / `git commit -a` を通さず、パス指定でしか
      組めない。他セッションの未コミット変更には触らせない（`stash` / `checkout --` / `reset --hard` は
      口に出さない）② **束ね方**（[commit-split](nimbus/docs/specs/commit-split.md)）とステージの状態を
      そのまま渡す ③ **承認と監査** — 押し上げのような外に出る操作は承認カードへ回し、監査に残す
      ④ **画面との連携** — 差分はエディタの差分ビューで見せる。
      口の案: `git_status`（自分の変更と他セッションのぶんを分けて返す）/ `git_stage`（パス指定のみ）/
      `git_commit`（メッセージ規約の検査つき・T-305 と繋ぐ）/ `git_sync`（`pull --rebase` → `push`・
      T-306 と同じ安全装置）。**履歴を壊す操作はツールに出さない**（force push・`reset --hard`）。
      どこまでを口にするかを決めてから実装する [P2] @session-e 2026-08-25

- [ ] T-308 **競合しているその場から Claude に相談できるようにする** — **中身は既にある**:
      コマンド「コンフリクトの解決を手伝う」（`nimbus.assistConflicts`・仕様
      [conflicts](nimbus/docs/specs/conflicts.md)）に「Claude に相談する」が入っていて、両側と
      （`diff3` なら）分岐元を貼った相談文をコックピットへ送る。足りないのは**入口**で、この
      コマンドは `editor/title` にも `editor/context` にも出ていない（T-294 と同じ原因）。
      VS Code 側の既存機能（インラインの こちら / むこう / 両方 と 3 方向のマージエディタ）は
      **置き換えない**。その隣に「Claude に相談」を並べる: ① 競合マーカーを含むファイルを開いたら
      **その競合の頭に CodeLens** を出す（`<<<<<<<` を見て出すので upstream に手を入れなくて済む）
      ② マージエディタのツールバーにも口を出せるか検討（貢献点があるかは要確認）
      ③ SCM の「マージの変更」からも呼べるように。約束は今までどおり — **返ってきた案を勝手に
      書き戻さない**、選ばれなかった競合はマーカーごと残す、保存と `git add` は人の手 [P2] @session-e 2026-08-25

- [ ] T-309 **コミットメッセージの型を指定できるようにする**（T-305 が使う）— 生成ボタンが
      何に合わせて書くかを、利用者が決められるようにする。選べる型は ① **このリポジトリの型**
      （日本語 1 行目 `Nimbus: 〜（T-xxx）`）② **Conventional Commits**（`feat(scope): …`）
      ③ **自由記述のテンプレート**。テンプレートの置き場所は**標準の口を使う** —
      `git config commit.template` を読む（作り直さない）。加えて設定で言語（日本語 / 英語）・
      1 行目の長さ・本文の有無・`Co-Authored-By` の扱いを決められるように。
      既定は**過去のコミットから型を数えて当てる**（`conventions.ts` と同じ考えかた —
      分かるものは推測させない。ただし conventions が数えているのは**コードの書き方**なので、
      コミットの型は別に数える）。生成の前に「いまの型」を画面に出し、違えば押す前に直せること [P2] @session-e 2026-08-25
