...
優先度や置き場所は後で決める。
- [ ] T-286 **コックピットを開いていないと、承認のたびにモーダルが出る** — 「いちいち POP が出る」の正体。承認は会話のカードで受ける作り（T-266）になっているが、`isLive()` は面が**生成済みか**を見ており、`reveal()` は `this.view?.show()` だけ。**別のアクティビティバー（タスク／設定／デバッグ）を見ているとコックピットの面は生成されていない**ので、`isLive()` が false になりモーダルへ落ちる。しかも `reveal()` も効かない（`view` が無いので）。素の送信では出ない（実測で確認済み）。直しかた: `reveal()` が面の無いときは `nimbus.cockpit.focus` でコンテナごと開く／モーダルへ落とす判断を**開き直したあと**に行う [P1]
