...
- [ ] T-299 **本物の Herdr と繋いで確かめる**（T-297 の残り）— 文書との突き合わせは済み
      （項目名のズレを 1 件直した）。残るのは実物の応答が文書どおりか・1.5 秒で足りるか・
      パッケージ版の一覧に同じ記号で混ざるか。**この機械に herdr が入っていない**ので、
      入れるかどうかは利用者の判断（同梱しない方針・T-280）@session-e 2026-08-20（利用者の了解を得て導入）[P3]
