...
優先度や置き場所は後で決める。
- [ ] T-289 **全画面・タブで開く導線が画面に無い** — `nimbus.fullscreenCockpit`（`$(screen-full)`）も `nimbus.openCockpitTab`（`$(link-external)`）もコマンドとしては在るのに、`view/title` に登録していないのでどこにも出ていない。**名前で引く経路しか無い機能は、知っている人にしか存在しない**（人間工学 E4）。コックピットとタスク板のタイトルバーにアイコンとして出す [P1]
- [ ] T-287 セッション中の折りたたみ（ツール・思考）が大きすぎる — 開いたときの中身が `max-height: 320px` あり、会話が押し流される。畳んだときの見た目・開いたときの上限・既定で開くかを見直す。実セッションを走らせて観察する [P2]
- [ ] T-296（旧 T-287・ID が重複したので採番し直し）**ドクターの赤が重複 57 件だけになった** — `test.sh` が毎回落ちるので、
      「赤いのが普通」になってしまい、**新しい指摘が埋もれる**（実際、ビューの偽の指摘 T-284 は
      その中に紛れていた）。中身は `同じ名前の export`（29 件）と `6 行以上そっくり同じ実装`（28 件）。
      潰すのか、意図したものを除外できるようにするのかを決める。`refactor` スキルの担当 [P2]
- [ ] T-297 **Herdr の組み込みを実機で突き合わせる**（T-279 の残り）— 実装・仕様・権利確認は入っている
      （`core/herdr.ts` / `src/herdr.ts` / 仕様 [herdr](nimbus/docs/specs/herdr.md)）が、確かめたのは
      **文書どおりに答える偽のソケット**までで、**本物の Herdr とは一度も突き合わせていない**。
      しかも**このマシンには herdr が入っていない**（`which herdr` が空）ので、「入っていれば使う」の
      経路は今のところ何も起きない。入れて確かめるのは ① ソケットの置き場所の実際
      （`HERDR_SOCKET_PATH` / `HERDR_SESSION` / `~/.config/herdr/herdr.sock`）② `agent.list` が実際に
      返す JSON の形と状態名の綴り・粒度 ③ 1.5 秒で諦める設定が実物で足りるか ④ パッケージ版の一覧に
      Herdr のぶんが同じ記号で混ざるか ⑤ **読むだけで操作していない**こと。
      ズレていたら `core/herdr.ts` の解釈と仕様書を直す [P2]

## 進行中

- [ ] T-290 **コックピットにセッションを増やす「+」が無い** — コマンド `nimbus.newSession` は
      あるのに、**アイコンが設定されておらず**（`package.json` の commands に `icon` 無し）、
      **ビューのタイトルにも出ていない**（`menus.view/title` にあるのは全画面とタブで開くの 2 つだけ）。
      押せる場所が無いので、コマンド名を知らないと新しいセッションを始められない。
      タブ列（T-269）の右端に「+」を置き、ビューのタイトルにも `$(add)` で出す。**全画面のときも押せる**こと [P1] @session-e 2026-08-20

- [ ] T-291 **セッションの中でモデルとエフォートを切り替えるボタンが無い** — 今あるのは
      `nimbus.agentModels`（**サブエージェント**のモデル割り当て・仕様
      [agent-models](nimbus/docs/specs/agent-models.md)）だけで、**いま話しているセッション自体**の
      モデルは変えられず、エフォートは触る口すら無い。材料は SDK に揃っている:
      `Query.setModel(model?)` で**走っている最中に**変えられ、`supportedModels()` が `displayName` /
      `description` に加えて `supportsEffort` / `supportedEffortLevels` / `supportsAdaptiveThinking` /
      fast mode まで返す（モデルごとに使えるエフォートが違うので、候補は必ずここから引く）。
      置き場所は入力欄の下の帯（枠の残り T-276 と同じ行）に「モデル」「エフォート」の 2 つ。
      検討が要るのは **エフォートを途中で変える口が SDK にあるか** — 無ければ次のターンや
      新しいセッションから効かせる形にして、いつから効くのかを画面に書く [P1] @session-e 2026-08-20

- [ ] T-292 **全画面にしても右半分（変更差分）が出ない** — 仕様
      （[cockpit-fullscreen](nimbus/docs/specs/cockpit-fullscreen.md) §右半分）では
      **コマンド「右半分に出すものを選ぶ（コマンドの出力 / 変更差分）」（`nimbus.showSessionSide`）で
      選んで初めて出る**作りになっていて、しかもこのコマンドは**どのビューのタイトルにも出ていない**
      （`menus.view/title` にあるのは全画面とタブで開くの 2 つ）。名前を知らないと出せないので、
      利用者からは「直したはずなのに出ない」に見える。**全画面にしたら既定で変更差分を出す**か、
      最低でも全画面のツールバーに `$(split-horizontal)` を置く。あわせて「出したのに空」の切り分けも
      要る — 差分に出るのは**そのセッションが書いたファイルだけ**で、読んだだけのファイルは出ない。
      実装 `src/sessionSide.ts` / `core/sessionSide.ts`・確認 GUI 46 [P1] @session-e 2026-08-20

- [ ] T-293 **走っているセッションの一覧を、押して開けるようにする** — コマンド
      「走っているセッションを見る」（`nimbus.showSessions`・アイコン `$(server-process)` 付き）は
      **あるのに押す場所が無い**。タブ列の横かコックピットのタイトルに置く。並列が増えるほど
      「今いくつ走っているか」を見たくなるので、件数のバッジも検討する（台帳 `sessionStore` は
      ウィンドウ横断で持っている）。T-277（セッションを探す入口）と口を分けるか兼ねるかは、そのとき決める [P2] @session-e 2026-08-20

- [ ] T-294 **横断: コックピットで使える操作を、コマンド名を知らなくても押せるようにする** — 同じ原因の
      報告が続けて 4 件出た（新規セッション T-290 / モデル・エフォート T-291 / 右半分 T-292 /
      セッション一覧 T-293）。**コマンドは足したが入口を作っていない**のが共通の原因で、
      `menus.view/title` に出ているのは全画面とタブで開くの 2 つだけ。コックピットから使う操作を
      一度棚卸しし、ツールバーと `...` に並べ直す。以後「コマンドを足したら入口も同じコミットで足す」を
      作法として書き残す [P2] @session-e 2026-08-20

- [ ] T-295 **枠の行に、数字だけでなくバーと絵文字を出す** — 入力欄の下の枠の行（T-282）は
      いま**素のテキスト 1 行**（`media/cockpit.js` の `quotaLine.textContent` ／ 文言は
      `core/usage.ts` の `quotaLine()`）。パーセンテージは残したまま、**残量が一目で分かるバー**を足す。
      使用量ビュー側には既に `███░░░░░░░` の擬似バー（`core/usage.ts` の `toGauges`）があるので、
      同じ見た目を借りるか、webview なので **CSS の細いバー**にするかを決める（後者のほうが幅を
      可変にできて滲まない）。色はテーマトークンから借り、残りが少ないときだけ警告色に寄せる。
      **絵文字も添える**（遊び心。ただし状態を絵文字**だけ**で表さない — 色だけに頼らないのと同じ理由で、
      数字・バー・絵文字の 3 つが同じことを言う形にする）。残量の帯で変える案 🟢🟡🔴 / ⏳ など。
      注意 2 つ: ① **1 行に収める** — T-282 で固めた `.app` の実機幅で末尾が切れた実績があるので、
      幅が伸びるものを足したらパッケージ版で必ず見る ② サイドバー幅のような狭いときはバーを畳む。
      和文と絵文字の混植で行の高さが跳ねないことも確かめる（`typography` スキル）[P2] @session-e 2026-08-20

- [ ] T-288 実セッションを走らせて UI を通しで見直す — 止まった絵ではなく、思考中・ツール実行中・承認待ち・完了、の各段階を撮って `design-philosophy` / `ergonomics` / `typography` で読む [P2] @session-e 2026-08-20

- [ ] T-298 **セッションの状態（完了 / 進行中 / あなたの番）がタブから読み取れない** — 作りとしては
      入っている（`media/cockpit.js` の `renderSessionTabs` が記号 `!●?✓■✕` ＋ テーマトークンの色を
      描く・仕様 [cockpit-fullscreen](nimbus/docs/specs/cockpit-fullscreen.md)）。読み取れない理由は
      3 つ考えられるので、実機で確かめてから直す: ① **1 本のときは列ごと出さない**作りなので、
      普段の 1〜2 本では状態が見えない（切り替え先が無くても状態は見たい — 列を出さないなら会話の上か
      枠の行の隣に出す）② 記号が小さく（`--nimbus-label2`）色もトークン任せで、パッと見で読めない
      （**絵文字と併せる** — T-295 と揃える。色・記号・文字の 3 つが同じことを言う形）
      ③ **状態の言葉がツールチップにしか無い**（幅があるときは文字でも出す）。
      あわせて更新経路も見る — 状態が変わった瞬間に描き直っているか。GUI ケース 47 は
      `--with-claude` 付きでしか走らないので、**普段のフル実行では状態表示の回帰を捕まえられない**。
      普段走る形の確認を 1 本足す [P1] @session-e 2026-08-20
