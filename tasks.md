...
- 🔒 @session-g | T-321 | 2026-08-25 20:32 | CLAUDE.md, README.md, nimbus/scripts/board.mjs, nimbus/tests/scripts/board.test.mjs
...
- [ ] T-321 **着手前にファイルを板で予約する運用を入れる** — 利用者指示（2026-08-25）。どのセッションも
      ...
      `board.mjs`（予約の表示＋モジュールテスト）を同じコミットで揃える @session-g 2026-08-25

- [ ] T-321 **現状を文書へ書き出す**（利用者依頼 2026-08-25）— README の「できること」「使いかた」が
      F1〜F6 期の記述のまま（タブ列・会話内承認・使用量の帯・MCP 3 系統・IntelliJ 由来・
      画面の置きかたの変更が未反映）。仕様書の索引も 2026-08-19 の棚卸し以降の 8 本
      （cockpit-home / commit-message / git-sync / git-tools / run-settings / session-health /
      setup-readiness / typography）が漏れている。直すのは ① README の柱の書き直し
      ② 索引の追補 ③ `nimbus/docs/README.md` を新設（文書の歩きかた＋ 2026-08-25 時点の全体像）。
      **コードは触らない**（文書のみ・固め直し不要）[P1] @session-g 2026-08-25
