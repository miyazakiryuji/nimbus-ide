...
- [ ] T-145 サブエージェントごとのモデル指定を GUI で設定する（軽いモデルで足りる仕事を分ける）[P2]
- [ ] T-146 タスクキューに優先度を持たせる（同時実行数の上限設定は実装済み）[P2]
...
- [ ] T-164 **秘匿ファイルの読み取り禁止リスト** — `.env` などを読ませない。
      T-155（文脈に入れない）とは別で、こちらは**読み取り自体を止める** [P1]
...
- [ ] T-167 モデル切り替えの比較 — 軽いモデルで足りるタスクを見つける（T-145 と対）[P2]
...
- [ ] T-185 **着手前の確認強制** — 曖昧な指示のまま走らせない [P1]
- [ ] T-186 **前提・仮定のリスト表示** — エージェントが置いた仮定を先に見せる [P1]
...
- [ ] T-120 ビルド設定の変更検知 — Gradle / Xcode 設定に触ったときは特に警戒して、差分を目立たせる [P2]
