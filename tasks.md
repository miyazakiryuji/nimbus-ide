...
優先度や置き場所は後で決める。
- [ ] T-291 **サイドバーで複数セッションのタブが破綻していた** — 実測（3 本立ち上げ）でサイドバー 299px に対し列が 567px に伸び、**前面のタブが画面外**へ隠れて「どのセッションに居るか」が分からなかった。`.session-tab` に `flex`／`min-width: 0` が無く縮まないため。縮むようにし、溢れても前面のタブを見える位置へ送る。あわせて状態の語が縦 1 列に積まれる（日本語は文字単位で折り返す）のも直す [P1]
- [ ] T-292 セッションタブの名前がプロンプト全文で、見分けられない — 3 本並べると 1 本あたり 86px しかなく、名前に使えるのは 30px 程度＝ほぼ「「…」だけ。**先頭数語ではなく、短い見出しを別に持つ**（要検討: 最初の名詞句を取る／利用者が付ける／連番）[P2]
- [ ] T-293 セッションの状態が絵文字（🟢 / ⚪ / ✓ / ?）で出ている — 他はすべて codicon なのでここだけ質感が違い、色もテーマに従わない（design-philosophy「Color comes from the theme」「Sameness signals sameness」）。枠の残りの帯（🟢 5 時間）も同じ [P2]
