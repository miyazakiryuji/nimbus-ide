...
優先度や置き場所は後で決める。
- [ ] T-276 **2 セッションが同時にビルドすると壊れる** — `CLAUDE.md` が「修正したら毎回固める」を求めているのに、出力先（`.build/extensions` / `out-vscode` / `Nimbus-darwin-arm64`）は 1 つしかない。実際に相手のビルドが途中で消したものを踏んで `ENOENT: .build/extensions/pug` で落ちた。**規約が事故を生む形になっている**ので直す。案: ビルド前に他の gulp を検出して待つ／ロックファイルで直列化する（T-250 の追記直列化と同じ話）[P1]
- [ ] T-275 コックピットの入力欄が 1 行ぶんしかない — `min-height: 22px` で、既定が 1 行。**主戦場の入力欄が検索欄に見える**うえ、2 行目を打った瞬間に下のツールバーごと動く（読んでいた場所を目で追い直すことになる・人間工学 E5）。既定を 3 行ぶんに上げる。伸びる上限（220px）はそのまま [P2]
...
- [ ] T-272 **コックピットの意匠を design-philosophy に合わせる** — 色はテーマ変数だけで組めていて、そこは良い。崩れているのは 4 点（値ではなく**役割**で直す）。**T-269 の作り替えと同じファイルなので、そちらが落ち着いてから当てる**
      ...
      3. **焦点色が飾りになっている** — 自分の発言と承認カードが `focusBorder` の縦線を**常時**着ている。焦点を表す色を装飾に使うと、本当に焦点があるときに区別がつかず、何も起きていないのに画面が声を上げる（*Quiet at rest* 1）
      4. **序列が逆** — いちばん強く見えるのが**自分が打った文**（塗り＋縦線）で、読みたい Claude の答えが素の文字。既に知っていることが目立ち、読むものが沈んでいる（*One thing leads* 4）
      あわせて、ターン同士の間隔（`gap: 8px`）が吹き出しの内側の余白と同じで、会話がひと塊に見える（*Room to breathe* 2）[P2]
...
- [ ] T-276 **枠の残り（5 時間 / 週）をコックピットの入力欄の下に常時出す** — いま使用量は
...
- [ ] T-279 **Herdr をセッションの土台に取り込めるか検討し、可能なら実装する** — Herdr
      （[herdr.dev](https://herdr.dev/) / [herdrdev/herdr](https://github.com/herdrdev/herdr)・Rust の
      単一バイナリ・Apache-2.0）は「コーディングエージェントが乗る実行環境」。**欲しいものが 3 つ揃っている**:
      ① 常駐サーバが端末を保持するのでウィンドウやラップトップを閉じてもセッションが生き残り、形
      （ワークスペース / タブ / ペイン / cwd / レイアウト）ごと復元できる（T-247 の持ち主・続きからに直結）
      ② エージェントの状態を **blocked / working / done / idle** の 4 つに判定して出す（T-269 ③ の
      タブの色そのもの）③ CLI とローカルソケット API があり外から操作できる。
      **大きな分岐**: Nimbus は今 SDK を拡張ホストのプロセス内で直接叩いている（`SessionManager` が
      `query()` を呼ぶ）のに対し、Herdr は端末の中で CLI を走らせる前提なので、そのまま入れると
      セッションの持ち主が二重になる。既存を壊さない原則から、まず**並べて置く**形（Herdr 側で起きている
      セッションを Nimbus が読んで一覧と状態に混ぜる）で試し、置き換えは別途判断する。同梱するのか
      「入っていれば使う」にするのかも検討に含める（外部バイナリの同梱は配布・署名・公証に効く）。
      **T-280 の権利確認が通るまで実装に入らない** [P1]
- [ ] T-280 **Herdr の権利まわりを確認する**（T-279 の前提）— ライセンスは Apache-2.0 と読めるが、
      確かめるのは ① 同梱するなら LICENSE / NOTICE の同梱義務を満たしているか ② **商標**（"Herdr" の
      名前をどこまで出してよいか）③ 依存の孫ライセンス（Rust の単一バイナリなので中身が見えない・
      配布物に何が入るか）④ Nimbus 自体（MIT の VS Code フォーク）との整合 ⑤ 全社の**第三者 OSS の
      扱い**（`00_hq`）に沿って台帳に登録する。**結論が出るまで取り込まない** [P1]
...
- [ ] T-274 **直したはずのものが戻る（回帰）を止める** — 実際に起きている（T-091 は一度直したあとに @session-d 2026-08-19
      戻り、`3d3be9a5a81` で直し直した）。原因は 3 つに分かれるので、対策もその 3 つに当てる:
      ① 並行セッションが古い前提でファイルを書き直す → **直した不具合には回帰テストを 1 本必ず足す**
      （ケース名に T 番号を入れ、どの修正を守っているか分かる形にする）② 直した振る舞いにテストが無く、
      壊れても緑のまま → 修正のたびにパッケージ版でフル実行する（CLAUDE.md「修正したらアプリとして
      固める」と対）③ upstream 追従でコアの Nimbus ブロックが落ちる → **doctor の台帳照合（§6）は
      いま警告止まり**なので、消えていたら赤で落ちる検査にする [P1]
- [ ] T-278（旧 T-275・ID が重複したので採番し直し）**過去に直したものが今も直っているかを、一度まとめて確かめる**（T-274 の初回棚卸し）— @session-d 2026-08-19
      完了済みのうち画面で見える振る舞いをパッケージ版で通しで確認し、戻っていたものを拾う。
      回帰テストが無いものはその場で足す [P2]

- [ ] T-269 **コックピットを全画面で使えるようにし、セッションをタブで切り替える** — 面は T-258 で @session-d 2026-08-19
      エディタタブに出せる（[editor-tabs](nimbus/docs/specs/editor-tabs.md)）ので、ここで足すのは 3 つ:
      ① **全画面**（サイドバー・パネルを畳んで会話だけにする）② **ファイルタブのような列でセッションを
      切り替える** ③ **タブの色で状態が分かる**（動作中 / Claude からの質問 / 許可待ち / 完了）。
      ③ は状態を 1 つに混ぜる必要がある — `SessionStatus` は `starting / running / awaiting-input /
      interrupted / completed / error` で**「許可待ち」を持っていない**（承認は `core/approvalQueue.ts`
      側の待ち行列）。色だけに頼らず記号も添える（色覚と、モノクロのスクショで潰れないため）。
      T-268 が同じ webview を作り直しているので、順番を合わせてぶつからないようにする [P1]
- [ ] T-270 **全画面モードの右半分に、ターミナルか変更差分を出す** — 左が会話（T-269）、右が @session-d 2026-08-19
      「いま何が起きているか」。出すのは 2 つのどちらか（切り替え）: **ターミナル**（セッションが動かした
      コマンドの出力）と **ソースの変更差分**（そのセッションが触ったファイルの diff）。
      **webview の中に描き直さない** — ターミナルも差分エディタもワークベンチが本物を持っているので、
      右のエディタグループに実物を置いて左右に並べる（`vscode.diff` / 既存の差分レビュー
      [approvals-and-diff](nimbus/docs/specs/approvals-and-diff.md) を使い回す）。
      セッションを切り替えたら右側も追従すること [P1]
