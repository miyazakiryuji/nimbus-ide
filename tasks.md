...
- [ ] T-276 **2 セッションが同時にビルドすると壊れる** — `CLAUDE.md` が「修正したら毎回固める」を求めているのに、出力先（`.build/extensions` / `out-vscode` / `Nimbus-darwin-arm64`）は 1 つしかない。実際に相手のビルドが途中で消したものを踏んで `ENOENT: .build/extensions/pug` で落ちた。**規約が事故を生む形になっている**ので直す。案: ビルド前に他の gulp を検出して待つ／ロックファイルで直列化する（T-250 の追記直列化と同じ話）[P1] @session-d 2026-08-19
