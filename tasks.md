...
- リモート開発の**実機での接続確認**（T-084 ②）— **Docker はこのマシンで動く**ので devcontainer なら道はあるが、リモート側で走る REH の linux 版ビルドが重く（数十分規模）、複数セッションが同じ作業ツリーでコンパイルしている間は回せない。加えて接続には Open VSX の OSS リモート拡張を先に入れる必要がある（薦める拡張を決めない方針なので、利用者の選択が要る）。経路と案内の実装（①③）は完了済み
