...
- [ ] T-274 **直したはずのものが戻る（回帰）を止める** — 実際に起きている（T-091 は一度直したあとに
...
- [ ] T-275 **過去に直したものが今も直っているかを、一度まとめて確かめる**（T-274 の初回棚卸し）—
...
- [ ] T-277 **セッションを探す入口を作る（まず検討し、決めてから実装まで）** — 並列が増えるほど
      「あのセッション」に戻れなくなる。素材は揃っている: 過去の記録は `nimbus.searchTranscripts`
      （コマンド・`~/.claude/projects/**` の JSONL を読む・仕様
      [transcript-search](nimbus/docs/specs/transcript-search.md)）、いま走っているぶんは台帳
      （`core/sessionRegistry.ts` / `sessionStore.ts`・ウィンドウ横断）、入口の作りは
      [search-everywhere](nimbus/docs/specs/search-everywhere.md)。足りないのは
      ① **いま走っているセッションを探す口**（過去の記録しか引けない）② **コックピットからの導線**
      （コマンド名を覚えていないと使えない）③ **絞り込み軸**（作業ディレクトリ / ブランチ / 触った
      ファイル / 状態 / 日付）④ **見つけたあと**（そのタブへ移る・続きから開く = T-252）。
      検討で決めるのは 2 つ — 置き場所（タブ列の横の検索欄 / Quick Pick / 全画面の左）と、
      **生きているセッションと過去の記録を 1 つの検索にまとめるか、分けるか**。
      T-269 と同じ面を触るので順番を合わせる [P1]
