...
優先度や置き場所は後で決める。
- [ ] T-268 **送信のたびにポップアップが出るのをやめる** — 曖昧さの確認が 2 段（`dialogue.confirmVaguePrompt` の T-185 と `clarifyVagueJapanese` の T-090）とも既定 on で、送るたびにモーダルが割り込む。**両方の既定を off に**する。設定は残すので、要る人は戻せる。**資格情報の検出（`safety.scanBeforeSend`）は既定 on のまま** — 外部へ鍵を送る事故を防ぐもので、毎回出るものではない [P1]
- [ ] T-268 **コックピットを VS Code のチャット画面の作りに寄せる** — 今の見た目は素の textarea と
      ...
      **コア側チャット（`src/vs/workbench/contrib/chat/` 932 ファイル）は使わない** — ワークベンチ内部
      ...
      ヘルプ（ゆあ）も同じ実装を使い回しているので、**両方が壊れないこと**を確認する [P1] @session-e 2026-08-19
- [ ] T-267 **承認待ち・レビュー・文脈が UI から消えていなかった** — `visibility: hidden` は**既定値**でしかなく、そのビューを一度見たことのあるプロファイルには効かない（VS Code が表示状態を覚えている）。新規プロファイルでは消えるので GUI テストは通り、**既存の利用者の画面にだけ残る**。`contributes.views` から外して、どのプロファイルでも出ないようにした。**コードは残す**（`registerTreeDataProvider` はビュー ID を検証しないので登録したままでも安全）[P1]
...
- [ ] T-269 **コックピットを全画面で使えるようにし、セッションをタブで切り替える** — 面は T-258 で
...
- [ ] T-270 **全画面モードの右半分に、ターミナルか変更差分を出す** — 左が会話（T-269）、右が
