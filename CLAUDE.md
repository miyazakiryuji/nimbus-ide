...
- **握られていたら**: 始めない。解放を待つか、重ならない別のタスクへ移るか、利用者に確認する。
  ...
  20〜30 分待っても解放されないときは、利用者へ報告して指示を仰ぐ
- **解放**: コミットと push が済んだら自分の 🔒 行を消す。タスクを `[x]` にするのと同じコミットでよい。
  **途中で作業を止めて利用者へ返すときも、握ったままにしない**（残りをタスク側に書いてから行を消す）
- 自分の行は途中で直してよい（ファイルを足す・済んだぶんを減らす）。**他人の 🔒 行は編集も削除も
  しない**。古い予約（目安 2 時間超）で対象ファイルに未コミット変更も無いときは、落ちたセッションの
  残骸かもしれない — 勝手に剥がさず、利用者に確認してから外す
