...
test('枠の目盛りは、バー・数字・印が同じことを言う（T-295 / T-302）', () => {
	const now = Date.parse('2026-08-19T10:00:00Z');
	assert.deepStrictEqual(
		quotaGauges(
			{
		...
		[
			{ label: '5 時間', left: 94, reset: '2 時間後にリセット', tone: 'ok' },
			// 残り 5% は危ない側。**バーも数字も印も**同じことを言う（印の色はテーマトークン）
			{ label: '週', left: 5, reset: '', tone: 'low' }
		]
	);
});
