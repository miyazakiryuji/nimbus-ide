...
type ManagedSettingsFetchTelemetryClassification = {
	...
	runtimeVersion: { classification: 'SystemMetaData'; purpose: 'PerformanceAndHealth'; comment: 'The bundled @github/copilot runtime version reported for compatibility evaluation.' };
	minimumClientVersion: { classification: 'SystemMetaData'; purpose: 'PerformanceAndHealth'; comment: 'The minimum client version returned by a managed-settings compatibility rejection.' };
};
...
export class DefaultAccountProvider extends Disposable implements IDefaultAccountProvider {
	...
	private reportManagedSettingsOutcome(status: Exclude<ManagedSettingsFetchStatus, null | 'no-url'>, rateLimitBackoffActive: boolean, compatibilityError?: IManagedSettingsCompatibilityError): void {
		...
		this.telemetryService.publicLog2<ManagedSettingsFetchTelemetry, ManagedSettingsFetchTelemetryClassification>('defaultaccount:managedSettings:fetch', {
			...
			runtimeVersion: this.productService.copilotVersions?.runtime,
			minimumClientVersion: compatibilityError?.minimumClientVersion,
		});
	}
