...
2. アクティビティバーの雲アイコン → **コックピット**に指示を書いて Enter
   ...
   診断は下部パネル「Nimbus 診断」、スキル・CLAUDE.md・設定はコマンドパレットから開きます）
...
**まず「制限モード（Restricted Mode）」を疑ってください。** リモートで開いたフォルダは
初回に信頼を聞かれ、答えるまで制限モードのままです。Nimbus は信頼されたフォルダでしか
動かない設定（`untrustedWorkspaces.supported: false`）なので、**制限モードでは
アクティビティバーにアイコンごと出ません**。「入れたのに何も起きない」ように見えます。

画面下の「制限モード」を押して、そのフォルダを信頼すると出ます。
実際に確認したときも、ここで一度つまずきました。
